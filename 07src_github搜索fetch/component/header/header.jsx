...
export default class header extends Component {
  ...
  search = () => {
    ...
    // axios
    //   .get(`http://192.168.1.10:3000/api/search/users?q=${keyWordNode.value}`)
    //   .then((res) => {
    //     console.log(res)
    ...
    fetch(`http://192.168.1.10:3000/api/search/users1?q=${keyWordNode.value}`)
      .then((res) => {
        console.log(res)
        if (!res.ok) {
      ...
      .then((data) => {
        PubSub.publish('userList', { isLoading: false, users: data.items })
      })
      .catch((err) => {
        console.error('搜索失败:', err)
        PubSub.publish('userList', { isLoading: false, err: err.message })
      })
  }
  render() {
    return (
